...
resource "aws_ecs_service" "ecs_service_backend_api" {
  name                               = local.ecs_backend_api_service_name
  cluster                            = aws_ecs_cluster.ecs_cluster.id
  ...
  load_balancer {
    target_group_arn = var.backend_api_target_group_arn # get tge value from output section of ALB aws_lb_target_group.backend_target_group.arn
...
resource "aws_ecs_task_definition" "backend_api" {
  family                   = local.ecs_family_backend_api
  requires_compatibilities = var.requires_compatibilities
  network_mode             = var.network_mode
  cpu                      = var.backend_api_cpu_usage
  memory                   = var.backend_api_memory_usage
  execution_role_arn       = aws_iam_role.task_execution_role.arn
  task_role_arn            = aws_iam_role.task_role.arn
  ...
  container_definitions = jsonencode([
    {
      name      = "backend"
      image     = "${var.backend_api_container_image_name}:latest"
      essential = true

      portMappings = [
        {
          protocol      = "tcp"
          containerPort = var.backend_api_port
          hostPort      = var.backend_api_port
        }
      ]
    }
  ])
  ...
  tags = merge(
  {
    "Name" = format("%s-%s-%s", var.app, "task-definition-backend-api", terraform.workspace)
  },
...
resource "aws_appautoscaling_target" "backend_api_target" {
  max_capacity       = 5
  min_capacity       = 1
  resource_id        = "service/${aws_ecs_cluster.ecs_cluster.name}/${aws_ecs_service.ecs_service_backend_api.name}"
  scalable_dimension = "ecs:service:DesiredCount"
  service_namespace  = "ecs"
}
...
resource "aws_appautoscaling_policy" "backend_api_scaling_cpu" {
  name               = "cpu-autoscaling"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.backend_api_target.resource_id
  scalable_dimension = aws_appautoscaling_target.backend_api_target.scalable_dimension
  service_namespace  = aws_appautoscaling_target.backend_api_target.service_namespace
